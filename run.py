...
def introduction():
    ...
    print(
        "This tool allows you to search for local hospitality businesses information based on their location."
    )
...
def get_contact_info(website_url):
    ...
    def scrape_page(url):
        ...
        try:
            ...
            possible_phones = re.findall(
                r"\b(?:\+?\d{1,3}\s*(?:\(0?\d{1,3}\))?|\(0?\d{1,3}\)\s*)?(?:(?:\d\s*){6,}\d)\b",
                soup.get_text(),
            )
            ...
            for script in scripts:
                ...
                if "address" in data:
                    additional_info["addressRegion"] = data["address"].get(
                        "addressRegion", ""
                    )
                if "starRating" in data:
                    additional_info["starRating"] = data["starRating"].get(
                        "ratingValue", ""
                    )
    ...
    try:
        ...
        for link in soup.find_all("a", href=True):
            href = link["href"]
            if any(
                keyword in href.lower() for keyword in ["contact", "about", "kontakt"]
            ):
    ...
    return (
        ...
        {"description": description, "Region": addressRegion, "Rating": starRating},
    )
    ...
    if "results" in search_result and search_result["results"]:
        for place in search_result["results"][:5]:
            ...
            business_info = [
                ...
                additional_info.get("addressRegion", ""),
                additional_info.get("starRating", ""),
            ]
    ...
    for business in businesses:
        try:
            ...
            if business[2] != "Not Available":
                print(
                    f"Business Name: {business[0]}, Email: {business[2]}, Phone: {business[3]}"
                )
        except Exception as e:
    ...
    while True:
        ...
        while not validated_location:
            location_input = input(
                "Enter the location (e.g., 'Cork, Ireland'): "
            ).strip()
            ...
            if location_response:
                ...
                confirm = (
                    input("Is this the correct location? (yes/no): ").strip().lower()
                )
                if confirm == "yes":
        ...
        another_search = (
            input("Would you like to perform another search? (yes/no): ")
            .strip()
            .lower()
        )
